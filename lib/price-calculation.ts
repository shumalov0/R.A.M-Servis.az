...
export const calculateRentalPrice = (
  ...
  if (days >= 30) {
    // Monthly pricing (most economical)
    ...
    if (remainingDays >= 7) {
      ...
      basePrice = (months * car.monthlyPrice) + (weeks * car.weeklyPrice) + (finalDays * car.dailyPrice);
    } else {
  ...
  const locationCharges = (pickupLocationData?.extraCharge || 0) + (dropoffLocationData?.extraCharge || 0);
  ...
  // Calculate weekend surcharge (10% extra for weekend days)
  const weekendDays = getWeekendDays(pickupDate, dropoffDate);
  const weekendSurcharge = weekendDays * car.dailyPrice * 0.1;

  // Calculate holiday surcharge (15% extra for holiday days)
  const holidayDays = getHolidayDays(pickupDate, dropoffDate);
  const holidaySurcharge = holidayDays * car.dailyPrice * 0.15;

  // Calculate subtotal before discounts
  const subtotal = basePrice + locationCharges + serviceCharges + weekendSurcharge + holidaySurcharge;
  ...
  // Calculate taxes (18% VAT in Azerbaijan)
  const taxableAmount = subtotal - discounts;
  const taxes = taxableAmount * 0.18;

  // Calculate final total
  const total = taxableAmount + taxes;

  // Deposit calculation (usually 20-30% of total or fixed amount, whichever is higher)
  const percentageDeposit = total * 0.25;
  const deposit = Math.max(car.deposit, percentageDeposit);
...
export const useRealTimePricing = () => {
  const calculatePrice = (car: EnhancedCar | null, options: Partial<PriceCalculationOptions>): PriceBreakdown | null => {
...
export const getPriceChangeNotifications = (
  ...
  if (newPrice.weekendSurcharge > 0) {
    notifications.push({
      type: 'info',
      message: `Weekend surcharge applied: ${formatPriceAZ(newPrice.weekendSurcharge)}`,
  ...
  if (newPrice.holidaySurcharge > 0) {
    notifications.push({
      type: 'info',
      message: `Holiday surcharge applied: ${formatPriceAZ(newPrice.holidaySurcharge)}`,
